/*
    ...
    -  It is foribidden to have an empty object; like empty string as data printed when we run the code;

    -  Always Object indicated and refers to an entity so it cant be an empty thing.

    -  So if we have an (address obj) of (address class) It should be filled and refers to a real life object

    -  We will try to block you from creating an empty object; Using OOP and Classes ومفاهيم برمجية

    -  (Constructor البناء (إسم مفعول)) word comes from (Construction : بناء)

    -  Constructor is always exist when you create any object of any class; ALways Created; 
        - If you dont define your constructor the compiler will do this (Automatically).

    -  Constructor is like a method that will be called; whenever you create object of class
        -   And always there is a default constructor.

    -   It is like a function  (Or can be called it is a function) - without datatype and have the same name of the class
            -   It also can not to take any parameter. cls_person(no_parameter) --> cls_person() 
